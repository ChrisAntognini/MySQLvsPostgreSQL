...
SELECT u AS id,
       ...
       ceiling(random()*5) AS normal1,
       ceiling(random()*500) AS normal2,
       ceiling(random()*50000) AS normal3,
...
CREATE INDEX t_scattered3_pi ON t(scattered3) WHERE scattered3 <= 500;
...
CREATE INDEX t_clustered3_pi ON t(clustered3) WHERE scattered3 <= 500;
...
CREATE INDEX t_normal3_pi ON t(normal3) WHERE normal3 <= 500;
...
CREATE STATISTICS s (ndistinct, dependencies) ON status1, status2, status3 FROM t;
...
SELECT attname, histogram_bounds FROM pg_stats WHERE tablename = 't' ORDER BY attname;
SELECT attname, most_common_vals FROM pg_stats WHERE tablename = 't' ORDER BY attname;
SELECT attname, most_common_freqs FROM pg_stats WHERE tablename = 't' ORDER BY attname;
...
SELECT relname, relpages, reltuples FROM pg_class WHERE relname LIKE 't%' AND reltype = 0;
...
EXPLAIN SELECT * FROM t WHERE status1 = FALSE;
EXPLAIN SELECT * FROM t WHERE status1 = TRUE;
EXPLAIN SELECT * FROM t WHERE status2 IS NULL;
EXPLAIN SELECT * FROM t WHERE status2 IS NOT NULL;
EXPLAIN SELECT * FROM t WHERE status2 = FALSE;
EXPLAIN SELECT * FROM t WHERE status2 = TRUE;
EXPLAIN SELECT * FROM t WHERE status3 IS NULL;
EXPLAIN SELECT * FROM t WHERE status3 IS NOT NULL;
EXPLAIN SELECT * FROM t WHERE status3 = FALSE;
EXPLAIN SELECT * FROM t WHERE status3 = TRUE;
